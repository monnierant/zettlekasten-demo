...
// src/Settings/SimpleRSSFeddPanel.ts
...
var SimpleRSSFeedPanel = class {
  ...
  display(containerEl) {
    ...
    this.plugin.settings.feeds.forEach((feed, index) => {
      ...
      new import_obsidian3.Setting(containerEl).setName("Type").addDropdown((dropdown) => {
        var _a, _b;
        dropdown.addOption("", "~Generic~").addOptions(
          this.plugin.settings.feedTypes.map((feedType) => {
            return {
              name: feedType.name,
        ...
        ).setValue((_b = (_a = feed.feedTypeId) == null ? void 0 : _a.toString()) != null ? _b : "").onChange(async (value) => {
          this.plugin.settings.feeds[index].feedTypeId = value;
          await this.plugin.saveSettings();
        });
      });
...
// src/Settings/SimpleRSSFeddTypePanel.ts
var import_obsidian4 = require("obsidian");
var SimpleRSSFeedTypePanel = class {
  constructor(app, plugin) {
    this.app = app;
    this.plugin = plugin;
  }
  display(containerEl) {
    ...
    new import_obsidian4.Setting(containerEl).addButton((button) => {
      button.setIcon("plus").setTooltip("Add feed type").onClick(async () => {
    ...
    this.plugin.settings.feedTypes.forEach((feedType, index) => {
      ...
      new import_obsidian4.Setting(containerEl).setName("Name").addText(
        (text) => text.setPlaceholder("Name").setValue(feedType.name).onChange(async (value) => {
          this.plugin.settings.feedTypes[index].name = value;
          await this.plugin.saveSettings();
        })
      );
      new import_obsidian4.Setting(containerEl).setName("Feed Field").addText((text) => {
        const feeds = feedType.feed.join(",");
        return text.setPlaceholder("Separator: ,").setValue(feeds).onChange(async (value) => {
          this.plugin.settings.feedTypes[index].feed = value.split(",");
          await this.plugin.saveSettings();
        });
      });
      new import_obsidian4.Setting(containerEl).setName("Item Field").addText((text) => {
        const items = feedType.item.join(",");
        return text.setPlaceholder("Separator: ,").setValue(items).onChange(async (value) => {
          this.plugin.settings.feedTypes[index].item = value.split(",");
          await this.plugin.saveSettings();
        });
      });
      new import_obsidian4.Setting(containerEl).addButton((button) => {
        button.setIcon("trash").setTooltip("Delete this feed type").onClick(async () => {
          console.log("Delete Feed");
          this.plugin.settings.feedTypes.splice(index, 1);
          this.plugin.settings.feeds = this.plugin.settings.feeds.map((feed) => {
            return {
              feedType: feed.feedTypeId !== feedType.id ? feed.feedTypeId : void 0,
              ...feed
            };
          });
          await this.plugin.saveSettings();
...
// src/Settings/SimpleRSSSettingTab.ts
var SimpleRSSSettingTab = class extends import_obsidian5.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  ...
  display() {
    ...
    new import_obsidian5.Setting(containerEl).setName("Pull").setDesc("Pull the feeds on periodic basis. (Restart required)").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.autoPull).onChange(async (value) => {
        this.plugin.settings.autoPull = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian5.Setting(containerEl).setName("Time interval").setDesc(
    ...
    new import_obsidian5.Setting(containerEl).setName("Default path").setDesc("This is the default path for all feed.").addSearch(
    ...
    new import_obsidian5.Setting(containerEl).setName("Default template").setDesc("This is the default template for all feed.").addTextArea(
...
var Feeds = class {
  constructor() {
    ...
    this.Mustache = require_mustache();
    this.feeds = [];
    this.feedTypes = [];
  }
  setFeeds(feeds) {
  ...
  syncFeeds(vault) {
    this.feeds.forEach((feed) => {
      const feedType = this.feedTypes.find(
        (feedType2) => feedType2.id === feed.feedTypeId
      );
      this.syncOneFeed(vault, feed, feedType);
    });
  }
  async syncOneFeed(vault, feed, feedType) {
    new import_obsidian6.Notice("Sync Feed: " + feed.name);
    ...
    content.items.forEach((item) => {
      ...
      vault.create(path + "/" + sanitizedTitle + ".md", text).then((file) => {
        ...
        new import_obsidian6.Notice("Note created :" + path + "/" + title);
      }).catch((error) => {
        if (!error.message.includes("File already exists")) {
          console.error(error);
          new import_obsidian6.Notice("Error creating note :" + error);
        }
      });
    });
  }
  getUrlContent(url, feedType) {
    ...
    const parser = new import_rss_parser.default(myFeedType);
  ...
  replaceField(template, kind, values) {
    const regex = new RegExp("{{" + kind + "\\.[^}]+}}", "g");
    let result = template;
    let match;
    match = regex.exec(result);
    match == null ? void 0 : match.forEach((m) => {
      var _a;
      const key = m.replace("{{" + kind + ".", "").replace("}}", "");
      result = result.replaceAll(m, (_a = this.getValue(values, key)) != null ? _a : "");
    });
    return result;
  }
  replaceFieldsMustache(template, values) {
...
// main.ts
var SimpleRSSPlugin = class extends import_obsidian7.Plugin {
  async onload() {
  ...
  async loadSettings() {
    ...
    this.feeds = new Feeds().setFeeds(this.settings.feeds).setFeedTypes(this.settings.feedTypes).setDefaultPath(this.settings.defaultPath).setDefaultTemplate(this.settings.defaultTemplate);
  }
  async saveSettings() {
